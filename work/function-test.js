...
assert(indexHtml.includes("app.js?v=0.57"), "script should use cache-busting version");
assert(indexHtml.includes("supabase-config.js?v=0.57"), "Supabase config should be loaded before the app");
...
assert(indexHtml.includes("v0.57"), "app shell should expose the new version");
...
assert(telegramFunction.includes('profileIds: ["angelika"]'), "Pap smear reminders should only be available to Angelika");
...
assert(fs.readFileSync("app.js", "utf8").includes('section.classList.contains("modal")'), "private modals should not auto-open after sign-in render");
assert(fs.readFileSync("app.js", "utf8").includes("telegramPanelOpen"), "Telegram modal should use explicit open state");
...
assert(indexHtml.includes("privacy-guard.js?v=0.57"), "privacy guard should be cache-busted");
assert(serviceWorker.includes("privacy-guard.js?v=0.57"), "privacy guard should be cached with the app shell");
...
assert(manifest.start_url.includes("v=0.57"), "manifest start URL should be cache-busted");
...
assert(manifest.icons.every((icon) => icon.src.includes("v=0.57")), "manifest icons should be cache-busted");
assert(indexHtml.includes("app-icon-180.png?v=0.57"), "iOS touch icon should use PNG");
assert(indexHtml.includes("health-dashboard-favicon.ico?v=0.57"), "browser favicon should use a unique Health Dashboard ICO filename");
assert(indexHtml.includes("health-dashboard-favicon-32.png?v=0.57"), "browser favicon should use a unique 32px PNG filename");
assert(indexHtml.includes("health-dashboard-favicon-16.png?v=0.57"), "browser favicon should use a unique 16px PNG filename");
...
assert(serviceWorker.includes("health-dashboard-v0.57"), "service worker cache should match app version");
assert(serviceWorker.includes("app.js?v=0.57"), "service worker should cache current app bundle");
assert(serviceWorker.includes("supabase-config.js?v=0.57"), "service worker should cache Supabase config placeholder");
assert(serviceWorker.includes("health-dashboard-favicon.ico?v=0.57"), "service worker should cache the unique ICO favicon");
assert(serviceWorker.includes("health-dashboard-favicon-32.png?v=0.57"), "service worker should cache the unique PNG favicon");
assert(serviceWorker.includes("app-icon-512.png?v=0.57"), "service worker should cache PNG app icons");
...
assert(document.elements.appVersion.textContent === "v0.57", "footer should show app version");
...
context.window.location.href = "https://benashy.github.io/health-tracker/index.html?v=0.57";
