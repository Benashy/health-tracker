...
function createDocument() {
  const ids = [
    ...
    "vitaminsOpenButton",
    "vitaminsCloseButton",
    "vitaminsProfileField",
    "vitaminsProfileInput",
    "vitaminsWeekStartInput",
    "vitaminsRedYeastField",
    "vitaminsRedYeastDateInput",
    "vitaminsCoq10FinishedField",
    "vitaminsCoq10FinishedInput",
    "vitaminsCopyButton",
    "vitaminsTodayButton",
    "vitaminsTimetableButton",
    "vitaminsPrintButton",
    "vitaminsCopyStatus",
    "vitaminsDoseFeedback",
    "vitaminsEmptyState",
    "vitaminsTodaySection",
    "vitaminsTodayTitle",
    "vitaminsTodaySubtitle",
    "vitaminsTodayList",
    "vitaminsMainSection",
    "vitaminsWeeklySection",
    "vitaminsMainBody",
    "vitaminsWeeklyBody",
    "exportCsvButton",
    "exportChatGptButton",
    ...
    "cancelBackupRestoreButton",
    "discardBackupRestoreButton",
    "confirmBackupRestoreButton",
    "metricContextModal",
    "metricContextTitle",
  ...
  ["importReviewModal", "backupRestoreModal", "metricContextModal", "telegramModal", "vitaminsModal"].forEach((id) => elements[id].classList.add("modal"));
  const classes = {
  ...
  return {
    ...
    querySelector(selector) {
      ...
      if (selector === ".modal:not(.hidden)") {
        return [elements.importReviewModal, elements.backupRestoreModal, elements.metricContextModal, elements.telegramModal, elements.vitaminsModal]
...
assert(fs.readFileSync("index.html", "utf8").includes("Import review file"), "import button should exist");
const indexHtml = fs.readFileSync("index.html", "utf8");
const manifest = JSON.parse(fs.readFileSync("manifest.webmanifest", "utf8"));
const serviceWorker = fs.readFileSync("service-worker.js", "utf8");
const privacyGuard = fs.readFileSync("privacy-guard.js", "utf8");
...
assert(indexHtml.includes("app.js?v=0.81"), "script should use cache-busting version");
assert(indexHtml.includes("supabase-config.js?v=0.81"), "Supabase config should be loaded before the app");
...
assert(appSource.includes("HEALTH_TRACKER_SUPABASE_CLIENT"), "main app should expose one shared Supabase client");
assert(privacyGuard.includes("HEALTH_TRACKER_SUPABASE_CLIENT"), "privacy guard should reuse the shared Supabase client");
...
assert(indexHtml.includes('class="brand-lockup"'), "Colour Clinical header should include a compact brand lockup");
assert(indexHtml.includes('class="brand-symbol"'), "Colour Clinical header should show the Health Dashboard mark");
assert(indexHtml.includes('class="summary-caution"'), "dashboard summary should expose a distinct caution surface");
assert(indexHtml.includes('class="summary-warning"'), "dashboard summary should expose a distinct warning surface");
assert(indexHtml.includes('class="summary-due"'), "dashboard summary should expose a distinct due surface");
...
assert(indexHtml.includes("entryDetailsToggle"), "measurement entry should have a details toggle");
assert(indexHtml.includes("entryDetailsPanel"), "measurement entry should have a collapsible details panel");
assert(indexHtml.includes("trackingControl"), "metric tracking control should exist in add measurement");
assert(indexHtml.includes("trackingToggleButton"), "metric tracking control should have a reversible toggle");
assert(indexHtml.includes("nextDueRelative"), "next due tile should include relative timing text");
assert(indexHtml.includes("schedulePagination"), "due soon panel should include pagination controls");
assert(indexHtml.includes("scheduleNextButton"), "due soon panel should have a next-page button");
assert(indexHtml.includes("reminderPlanner"), "due soon panel should include a reminder planning summary");
...
assert(indexHtml.includes("vitaminsTodayButton"), "Vitamins organiser should have a Today button");
assert(indexHtml.includes("vitaminsTimetableButton"), "Vitamins organiser should have a Timetable button");
assert(indexHtml.includes("vitaminsTodaySection"), "Vitamins organiser should have a compact Today view");
...
assert(indexHtml.includes("Weekly timetable"), "Vitamins organiser should include the weekly timetable");
assert(indexHtml.includes("Expanded schedule"), "Vitamins organiser should include the expanded schedule");
assert(indexHtml.indexOf("Weekly timetable") < indexHtml.indexOf("Expanded schedule"), "Vitamins weekly timetable should appear before the expanded schedule");
assert(indexHtml.includes("vitaminsDoseFeedback"), "Vitamins organiser should include a dosage feedback area");
assert(indexHtml.includes("v0.81"), "app shell should expose the new version");
assert(styles.includes("--info: #2d72a6"), "interactive hover colour should use the Colour Clinical blue");
assert(styles.includes("--accent: #1d7968"), "Colour Clinical should use the richer green brand accent");
assert(styles.includes("Colour Clinical visual system"), "Colour Clinical visual overrides should be present");
...
assert(styles.includes(".tracking-control"), "metric tracking control should have compact styling");
assert(styles.includes(".metric-not-tracked"), "stopped metrics should use a distinct form state");
assert(styles.includes(".entry-details-toggle"), "measurement details should have a compact disclosure style");
assert(styles.includes(".result-card-grid"), "current results should have compact card styling");
assert(styles.includes(".reminder-plan-grid"), "reminder planning should have compact card styling");
assert(styles.includes(".schedule-pagination"), "due soon pagination should have compact styling");
assert(styles.includes(".vitamins-modal-panel"), "Vitamins organiser should have modal styling");
assert(styles.includes("@media print"), "Vitamins organiser should include a print-friendly view");
...
assert(telegramFunction.includes("Snooze all 1d"), "Telegram reminders should include a 1-day snooze button");
...
assert(telegramFunction.includes('["1", "3", "7"]'), "Telegram callback parsing should accept 1-day snooze actions");
assert(telegramFunction.includes("callback_data"), "Telegram reminders should include inline button callback data");
assert(!fs.readFileSync("app.js", "utf8").includes("data-snooze-profile"), "dashboard due cards should not expose app-level snooze buttons");
...
assert(telegramFunction.includes("getMetricTrackingByProfile"), "Telegram reminders should respect per-user metric tracking preferences");
assert(telegramFunction.includes('!== "not_tracked"'), "Telegram reminders should skip not-tracked metrics");
...
assert(indexHtml.includes("primaryNavigation"), "desktop and tablet should have focused primary navigation");
...
assert(indexHtml.includes("resultsCardGrid"), "results should have a compact card surface for everyday use");
assert(indexHtml.includes("measurement-value-control"), "measurement value and unit should have a clear combined control");
...
assert(indexHtml.includes("privacy-guard.js?v=0.81"), "privacy guard should be cache-busted");
assert(serviceWorker.includes("privacy-guard.js?v=0.81"), "privacy guard should be cached with the app shell");
...
assert(manifest.start_url.includes("v=0.81"), "manifest start URL should be cache-busted");
...
assert(manifest.icons.every((icon) => icon.src.includes("v=0.81")), "manifest icons should be cache-busted");
assert(indexHtml.includes("app-icon-180.png?v=0.81"), "iOS touch icon should use PNG");
assert(indexHtml.includes("health-dashboard-favicon.ico?v=0.81"), "browser favicon should use a unique Health Dashboard ICO filename");
assert(indexHtml.includes("health-dashboard-favicon-32.png?v=0.81"), "browser favicon should use a unique 32px PNG filename");
assert(indexHtml.includes("health-dashboard-favicon-16.png?v=0.81"), "browser favicon should use a unique 16px PNG filename");
...
assert(serviceWorker.includes("health-dashboard-v0.81"), "service worker cache should match app version");
assert(serviceWorker.includes("app.js?v=0.81"), "service worker should cache current app bundle");
assert(serviceWorker.includes("supabase-config.js?v=0.81"), "service worker should cache Supabase config placeholder");
assert(serviceWorker.includes("health-dashboard-favicon.ico?v=0.81"), "service worker should cache the unique ICO favicon");
assert(serviceWorker.includes("health-dashboard-favicon-32.png?v=0.81"), "service worker should cache the unique PNG favicon");
assert(serviceWorker.includes("app-icon-512.png?v=0.81"), "service worker should cache PNG app icons");
...
assert(styles.includes(".app-view-hidden"), "all screen sizes should use focused workspace hiding");
assert(styles.includes(".mobile-menu-panel"), "styles should include a mobile menu screen");
assert(styles.includes(".result-compact-item"), "result rows should use expandable compact styling");
assert(styles.includes(".trend-chart-primary"), "trend chart should be the primary trend surface");
...
assert(styles.includes("max-height: calc(100vh - 40px)"), "desktop measurement entry should be scrollable inside its sticky panel");
assert(styles.includes("@media (max-width: 1100px)"), "styles should include an iPad/tablet refinement breakpoint");
assert(styles.includes(".vitamins-today-list"), "Vitamins Today view should have compact responsive styling");
...
assert(document.elements.appVersion.textContent === "v0.81", "footer should show app version");
...
document.elements.vitaminsModal.classList.remove("hidden");
...
assert(document.elements.vitaminsModal.classList.contains("hidden"), "signed-out privacy decision should close Vitamins organiser");
document.elements.contentGrid.classList.add("hidden");
document.elements.telegramModal.classList.add("hidden");
document.elements.vitaminsModal.classList.add("hidden");
...
assert(document.elements.vitaminsModal.classList.contains("hidden"), "signed-in privacy decision should keep Vitamins closed");
context.setPrivateVisibility(false);
assert(
...
context.window.location.href = "https://benashy.github.io/health-tracker/index.html?v=0.81";
...
assert(context.getDueStatus(context.getProfile("angelika"), context.getMetric("Total testosterone", "angelika")).label === "Baseline due", "active metrics with no data should show as baseline due");
vm.runInContext('state.scheduleState.snoozes["angelika:Total testosterone"] = { due_date: "2099-01-01", source: "telegram" };', context);
assert(context.getDueStatus(context.getProfile("angelika"), context.getMetric("Total testosterone", "angelika")).label === "Baseline due", "Telegram snoozes should not hide due status in the dashboard");
vm.runInContext('delete state.scheduleState.snoozes["angelika:Total testosterone"];', context);
document.elements.personInput.value = "angelika";
document.elements.markerInput.value = "Total testosterone";
context.syncMetricDefaults();
assert(document.elements.trackingStatusText.textContent.includes("Tracked"), "selected metrics should show tracked status by default");
context.toggleSelectedMetricTracking();
assert(context.getMetricTrackingStatus("angelika", "Total testosterone") === "not_tracked", "Angelika should be able to stop tracking testosterone");
assert(document.elements.trackingStatusText.textContent.includes("Not tracked"), "stopped metrics should show not tracked status");
assert(document.elements.resultForm.classList.contains("metric-not-tracked"), "stopped metrics should put the entry form into not-tracked mode");
context.populateMetrics();
assert(document.elements.markerInput.innerHTML.includes("Total testosterone (not tracked)"), "stopped metrics should remain available for re-tracking in the dropdown");
assert(!context.getScheduleItems("angelika").some((item) => item.metric.name === "Total testosterone"), "stopped metrics should disappear from due checks");
vm.runInContext('state.results.push(normaliseResult({ profile_id: "angelika", person_name: "Angelika", metric: "Total testosterone", sample_date: "2026-07-01", test_date: "2026-07-01", result_value: 28, unit: "ng/dL" }));', context);
assert(!context.visibleResults().some((result) => result.profile_id === "angelika" && result.metric === "Total testosterone"), "stopped metrics should hide historical rows from active results");
context.toggleSelectedMetricTracking();
assert(context.getMetricTrackingStatus("angelika", "Total testosterone") === "active", "stopped metrics should be restartable");
assert(context.getScheduleItems("angelika").some((item) => item.metric.name === "Total testosterone"), "re-tracked metrics should return to due checks");
vm.runInContext('state.results = state.results.filter((result) => !(result.profile_id === "angelika" && result.metric === "Total testosterone"));', context);
...
const firstSchedulePageText = document.elements.schedulePageText.textContent;
const firstSchedulePageHtml = document.elements.schedulePanel.innerHTML;
assert(/^1-8 of \d+$/.test(firstSchedulePageText), "due soon should show the first page count when many checks are due");
document.elements.scheduleNextButton.listeners.click();
assert(/^9-\d+ of \d+$/.test(document.elements.schedulePageText.textContent), "due soon next button should show the second page count");
assert(document.elements.schedulePanel.innerHTML !== firstSchedulePageHtml, "due soon second page should show a different set of checks");
document.elements.schedulePrevButton.listeners.click();
assert(document.elements.schedulePageText.textContent === firstSchedulePageText, "due soon previous button should return to the first page");
...
assert(document.elements.primaryNavigation.innerHTML.includes('data-primary-action="home"'), "desktop navigation should render Home");
assert(document.elements.primaryNavigation.innerHTML.includes('data-primary-action="results"'), "desktop navigation should render Results");
assert(
...
assert(context.getVitaminItems("ben").find((item) => item.item === "Fexofenadine").quantity === "120 mg tablet", "Ben's fexofenadine dose should show as 120 mg");
assert(context.getVitaminItems("ben").some((item) => item.item === "IM8 Daily Essentials"), "Ben should have IM8 Daily Essentials in the vitamin organiser");
assert(context.getVitaminItems("ben").some((item) => item.item === "IM8 Daily Longevity"), "Ben should have IM8 Daily Longevity in the vitamin organiser");
assert(context.getVitaminItems("ben").find((item) => item.item === "IM8 Daily Essentials").timing === "Breakfast", "IM8 Daily Essentials should sit with breakfast after fexofenadine");
assert(context.getVitaminItems("ben").find((item) => item.item === "IM8 Daily Longevity").timing === "Breakfast", "IM8 Daily Longevity should sit with breakfast after fexofenadine");
assert(context.getVitaminItems("ben").some((item) => item.item === "Lion's Mane"), "Ben's Lion's Mane entry should avoid brand clutter");
assert(!context.getVitaminItems("ben").some((item) => item.item.includes("Zenement")), "Ben's Lion's Mane entry should not show the brand name");
...
assert(
  context.getVitaminItemsForDay("ben", context.getVitaminItems("ben"), "tuesday").flatMap((group) => group.items).some((item) => item.item === "Red Yeast Rice"),
  "Tuesday's Today view should include Red Yeast Rice",
);
assert(
  !context.getVitaminItemsForDay("ben", context.getVitaminItems("ben"), "tuesday").flatMap((group) => group.items).some((item) => item.item === "CoQ10"),
  "Today view should exclude optional CoQ10",
);
assert(context.getVitaminItems("ben").some((item) => item.item.includes("IM8")), "IM8 brand name should show in the organiser labels");
...
context.handleMenuAction("vitamins");
...
assert(document.elements.vitaminsMainBody.innerHTML.includes("120 mg tablet"), "Vitamins organiser should render the fexofenadine dose");
assert(document.elements.vitaminsMainBody.innerHTML.includes("IM8 Daily Essentials"), "Vitamins organiser should render IM8 Daily Essentials");
assert(document.elements.vitaminsMainBody.innerHTML.includes("IM8 Daily Longevity"), "Vitamins organiser should render IM8 Daily Longevity");
...
assert(document.elements.vitaminsWeeklyBody.innerHTML.includes("data-vitamin-dose"), "Weekly vitamin items should be clickable for dosage");
assert(!document.elements.vitaminsWeeklySection.classList.contains("hidden"), "Timetable should show the weekly table by default");
assert(!document.elements.vitaminsMainSection.classList.contains("hidden"), "Timetable should show the expanded schedule by default");
assert(document.elements.vitaminsTimetableButton.classList.contains("active"), "Timetable button should be active by default");
context.showVitaminsTodayView();
assert(!document.elements.vitaminsTodaySection.classList.contains("hidden"), "Today button should reveal a compact daily vitamin view");
assert(document.elements.vitaminsWeeklySection.classList.contains("hidden"), "Today view should hide the weekly timetable");
assert(document.elements.vitaminsMainSection.classList.contains("hidden"), "Today view should hide the expanded schedule");
assert(document.elements.vitaminsTodayButton.classList.contains("active"), "Today button should be active in Today view");
assert(document.elements.vitaminsTodayTitle.textContent.includes("Today:"), "Today view should name the current weekday");
assert(document.elements.vitaminsTodayList.innerHTML.includes("Fexofenadine"), "Today view should include daily tablets");
assert(document.elements.vitaminsTodayList.innerHTML.includes("120 mg tablet"), "Today view should show the fexofenadine dose");
assert(document.elements.vitaminsTodayList.innerHTML.includes("IM8 Daily Essentials"), "Today view should include IM8 Daily Essentials");
assert(document.elements.vitaminsTodayList.innerHTML.includes("IM8 Daily Longevity"), "Today view should include IM8 Daily Longevity");
assert(document.elements.vitaminsTodayList.innerHTML.indexOf("Fexofenadine") < document.elements.vitaminsTodayList.innerHTML.indexOf("IM8 Daily Essentials"), "Today view should show fexofenadine before breakfast IM8 items");
assert(document.elements.vitaminsTodayList.innerHTML.includes("after the fexofenadine gap"), "Today view should show the fexofenadine timing note for IM8 items");
assert(document.elements.vitaminsTodayList.innerHTML.includes("data-vitamin-dose"), "Today view items should be clickable for dosage");
assert(document.elements.vitaminsTodayButton.textContent === "Today", "Today button should keep a stable label");
context.showVitaminsTimetableView();
assert(document.elements.vitaminsTodaySection.classList.contains("hidden"), "Timetable view should hide the compact Today panel");
assert(!document.elements.vitaminsWeeklySection.classList.contains("hidden"), "Timetable view should restore the weekly timetable");
assert(!document.elements.vitaminsMainSection.classList.contains("hidden"), "Timetable view should restore the expanded schedule");
assert(document.elements.vitaminsTimetableButton.classList.contains("active"), "Timetable button should be active in Timetable view");
context.showVitaminDoseFeedback("Fexofenadine: 120 mg tablet");
assert(document.elements.vitaminsDoseFeedback.textContent === "Fexofenadine: 120 mg tablet", "clicking a vitamin should show its dosage");
...
assert(context.formatReminderNotice(context.getMetric("Colonoscopy", "angelika")) === "4 months before", "colonoscopy reminder plan should explain the long booking window");
assert(context.formatTelegramMilestones(context.getMetric("Pilot medical", "ben")).includes("6w"), "pilot medical reminder plan should include the six-week milestone");
assert(document.elements.reminderPlanner.innerHTML.includes("Reminder plan"), "dashboard should render the reminder planning summary");
...
assert(!document.elements.entryDetailsPanel.classList.contains("hidden"), "first LDL setup should open the entry details panel");
assert(document.elements.entryModeTitle.textContent === "Set references", "first LDL setup should explain that references are needed");
...
assert(document.elements.resultsCardGrid.innerHTML.includes("LDL"), "current results should render compact result cards");
assert(document.elements.entryDetailsPanel.classList.contains("hidden"), "repeat LDL entry should collapse optional details after the range is saved");
assert(document.elements.saveFeedback.textContent === "LDL saved to your dashboard.", "saving should show clear measurement feedback");
...
assert(!document.elements.entryDetailsPanel.classList.contains("hidden"), "editing a saved range should reopen the entry details panel");
...
assert(context.daysBetween(context.parseDateString("2026-03-28"), context.parseDateString("2026-03-30")) === 2, "date-only intervals should not change across daylight-saving weekends");
assert(context.toDateString(context.addMonths(context.parseDateString("2026-01-31"), 1)) === "2026-02-28", "month recurrence should clamp safely at month end");
assert(context.toDateString(context.addMonths(context.parseDateString("2024-01-31"), 1)) === "2024-02-29", "month recurrence should preserve leap-day month ends");
assert(context.isValidDateString("2026-02-29") === false, "invalid calendar dates should be rejected");

const allDashboardData = context.serializeDashboardData();
const benDashboardData = {
  ...allDashboardData,
  profiles: allDashboardData.profiles.filter((profile) => profile.id === "ben"),
  measurements: allDashboardData.measurements.filter((measurement) => measurement.profile_id === "ben"),
  reference_ranges: Object.fromEntries(Object.entries(allDashboardData.reference_ranges).filter(([key]) => key.startsWith("ben:"))),
  schedule_state: { snoozes: {} },
  data_version: 2,
};
const fullBackup = context.createFullBackupPayload(benDashboardData);
assert(fullBackup.backup_type === "health_dashboard_full_backup", "full backup should identify its recovery format");
assert(fullBackup.data.measurements.length > 0, "full backup should include dashboard measurements");
vm.runInContext('state.activeProfileId = "ben";', context);
const restoreReview = context.prepareFullBackupRestore(fullBackup);
assert(restoreReview.profileName === "Ben", "restore review should confirm the target account");
assert(restoreReview.measurementCount === benDashboardData.measurements.length, "restore review should count every measurement");
let rejectedOtherProfileRange = false;
try {
  context.prepareFullBackupRestore({
    ...fullBackup,
    data: { ...benDashboardData, reference_ranges: { ...benDashboardData.reference_ranges, "angelika:LDL": { high: 3 } } },
  });
} catch {
  rejectedOtherProfileRange = true;
}
assert(rejectedOtherProfileRange, "restore should reject saved ranges belonging to another profile");
let rejectedFutureVersion = false;
try {
  context.migrateDashboardData({ data_version: 999 });
} catch {
  rejectedFutureVersion = true;
}
assert(rejectedFutureVersion, "future dashboard data versions should be rejected rather than guessed");

const firstResultId = vm.runInContext("state.results[0].id", context);
const resultCountBeforeDelete = JSON.parse(store["blood-results-tracker:v3"]).length;
context.deleteResult(firstResultId);
assert(JSON.parse(store["blood-results-tracker:v3"]).length === resultCountBeforeDelete - 1, "deleting a measurement should remove it immediately");
assert(!document.elements.undoToast.classList.contains("hidden"), "deletion should offer a visible undo action");
context.undoDeleteResult();
assert(JSON.parse(store["blood-results-tracker:v3"]).length === resultCountBeforeDelete, "undo should restore the deleted measurement");

assert(indexHtml.includes("Download full backup"), "data menu should expose a portable full backup");
assert(indexHtml.includes("Restore full backup"), "data menu should expose validated full restore");
assert(appSource.includes('.eq("updated_at", cloudState.cloudUpdatedAt)'), "browser cloud saves should use the loaded update token");
assert(appSource.includes("savePromise") && appSource.includes("saveQueued"), "browser cloud saves should serialise queued edits");
assert(telegramFunction.includes("updated_at=eq."), "Telegram snoozes should use an update token");
assert(telegramFunction.includes("for (let attempt = 0; attempt < 3"), "Telegram snoozes should retry bounded conflicts");
assert(indexHtml.includes('lang="en-GB"'), "document language should match the British interface copy");
assert(indexHtml.includes('aria-labelledby="backupRestoreTitle"'), "restore review should be an accessible dialog");
